...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!["csv", "json"].includes(ext ?? "")) {
      return NextResponse.json({ error: "Unsupported file type. Upload a .csv or .json file." }, { status: 400 });
    }
    ...
    catch { return NextResponse.json({ error: "Could not read file content." }, { status: 400 }); }

    if (!content.trim()) return NextResponse.json({ error: "File is empty." }, { status: 400 });

    // ── Parse raw rows ─────────────────────────────────────────────────────
    let rawRows;
    try { rawRows = parseFile(content, filename); }
    catch (e: unknown) {
    ...
    const summary = calculateSummary(annotated, sessionId);
